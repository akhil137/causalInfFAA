...
\title{Causal Inference for ATM Decision Making}
...
% in the abstract
\begin{abstract}
We introduce the ideas of Causal Inference to statistically estimate counterfactual outcomes, by definition never observable, of Traffic Flow Management Initiatives (TFMI), as a novel means to improve Air Traffic Management (ATM) decision-making.  Specifically we apply the method of Propensity Scores (PS) to estimate the average outcome of not implementing a Ground Delay Program (GDP) to flights that were affected by a GDP, thus quantifying the counterfactual and enabling 'what-if' analysis for an ATM decision aid which could be used to tradeoff TFMI options.  Furthermore we introduce a new metric, capacity-adjusted airborne delay, which we argue should be used to quantify the outcome of a GDP.  Future extensions of our proof-of-concept application of causal inference to ATM, some of which are underway, are also discussed.
\end{abstract}
