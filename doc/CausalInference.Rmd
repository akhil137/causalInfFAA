...
* Unbiased counterfactual estimation requires eliminating confounders
    * confounder: covariate which affects treatment status (GDP or no-GDP) and outcome (Airborne delay)
...
* There are two sets of arrival counts in ASPM: Hourly and Quarter-Hourly (from TAER module)
* The TAER module provides: ARRDEMAND (expected arrival demand) and EFFARR (counts of wheels on ground)
* The hourly arrivals counts are highly correlated with EFFARR
* We define Queue Length = ARRDEMAND - EFFARR and aggregate these quarter hour counts of ``unserved" aircraft up to the hour
...
#Is ql a strong predictor of forecasted GDP events too?

#which component of ql (ARRDEMAND or EFFARR) is the strongest predictor?
* let's include both in the model

#A stochastic process model for EFFARR
* cite heathrow
